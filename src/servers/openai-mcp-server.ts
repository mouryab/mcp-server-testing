...
export const toolDefinitionsOpenAIMCPServer = [
	{
		name: "search",
		description:
			"Tool to search for relevant data queries to answer the given question based on the datasource passed to this tool, which is a datasource id, see the query description for the syntax. The datasource id is mandatory and should be passed as part of the query. Any textual question can be passed to this tool, and it will do its best to find relevant data queries to answer the question.",
	...
	},
	{
		name: "fetch",
		description:
			"Tool to retrieve data from the retail sales dataset for a given query.",
...
export class OpenAIDeepResearchMCPServer extends BaseMCPServer {
	...
	protected async listTools() {
		return {
			tools: [...toolDefinitionsOpenAIMCPServer],
		};
	}
	...
	protected async callSearch(request: z.infer<typeof CallToolRequestSchema>) {
		...
		if (datasourceId) {
			const relevantQuestions =
				await this.getThoughtSpotService().getRelevantQuestions(
					queryWithoutDatasourceId,
					[datasourceId],
					"",
				);
			if (relevantQuestions.error) {
				return this.createErrorResponse(
					relevantQuestions.error.message,
					`Error getting relevant questions ${relevantQuestions.error.message}`,
				);
			}
		...
		const dataSources =
			await this.getThoughtSpotService().getDataSourceSuggestions(
				queryWithoutDatasourceId,
			);
